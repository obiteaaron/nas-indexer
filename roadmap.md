...
| 优先级 | 任务 | 状态 | 说明 |
|--------|------|------|------|
| **P0** | 智能文件预览增强 | 待开始 | 在列表页，在文件名前增加一列缩略图，如果是图片，则直接显示；如果是视频，则通过浏览器预览获取到第一帧直接显示；时长、分辨率、编码格式、码率如果浏览器能获取到也最好 |
| **P1** | 导出功能 | 待开始 | 增加导出文件列表的明细Excel即可 |
| **P2** | 浏览器通知 | 待开始 | 扫描任务完成时浏览器通知 |
