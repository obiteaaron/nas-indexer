...
### 文档
- **群晖 NAS 部署指南** - 新增 DEPLOYMENT.md，完整 Docker 部署方案
  - 三种部署方案对比（Docker、Node.js 直接运行、pkg 打包）
  - 详细 Docker 部署步骤（镜像构建、传输、加载、启动）
  - 目录挂载、端口配置、更新部署说明
  - 常见问题解答和资源占用参考
...
- **前端 UI 阻塞问题修复** - 解决异步操作时按钮无响应的问题
  - 新增全局请求状态管理 composable (`useRequestState.ts`)
...
### 功能改进
- **定时扫描控制** - 支持通过空值关闭定时扫描
  - 设置 Cron 表达式为空或空白字符串可关闭定时扫描
  - 无效 Cron 表达式自动被捕获并禁用定时任务
  - 前端提示更新，说明空值可关闭功能
